...
class VC:
    ...
    def vc_single(
        ...
        try:
            audio = load_audio(input_audio_path, 16000)
            logger.info(f"Loaded audio from {input_audio_path}, length: {len(audio)} samples")
            ...
            logger.info(f"Using index file: {file_index if file_index else 'None'}")
            ...
            logger.info(f">>> VC Success: {index_info}\nTime:\nnpy {times[0]:.2f}s\nf0 {times[1]:.2f}s\ninfer {times[2]:.2f}s\npipeline: {pipeline_time:.2f}s")

            output_path = "/Users/jack/Desktop/output/test.wav"
            logger.info(f">>> Start writing output to {output_path}")
            write_start = time.time()
            write(output_path, tgt_sr, audio_opt)
            writetime = time.time() - write_start
            logger.info(f">>> End writing, time: {writetime:.2f}s")
